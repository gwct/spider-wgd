...
treeToDF <- function (tree, tree_type="object", add_avg_bl=FALSE) {
  ...
  if ("edge.length" %in% names(tree) ){
    ...
    if(add_avg_bl){
      ...
      for(i in 1:length(tree[["edge.length"]])){
        if(!is.na(tree[["edge.length"]][i]) && !is.nan(tree[["edge.length"]][i])){
          orig_blens = c(orig_blens, tree[["edge.length"]][i])
        }
      }
      avg_blen = mean(orig_blens)
    }

    for(i in 1:length(tree[["edge.length"]])){
      if(add_avg_bl){
        if(is.na(tree[["edge.length"]][i]) || is.nan(tree[["edge.length"]][i])){
          tree[["edge.length"]][i] = avg_blen
        }
      }
