...
in_data = rbind(in_data16, in_data19)
...
single_copy = subset(in_data, label=="16 species" & genes==16 & taxa==16 | label=="19 species" & genes==19 & taxa==19)
...
dist_p = ggplot(dist_data, aes(x=genes, fill=label)) +
  ...
  #scale_y_continuous(expand=c(0,0), limits=c(0,max(single_copy$single.copy)+100)) +
  scale_fill_manual(values=corecol(pal="wilke", numcol=2)) +
  ylab("# of clusters") +
  xlab("Cluster size") +
  bartheme() +
  theme(legend.position="bottom")
